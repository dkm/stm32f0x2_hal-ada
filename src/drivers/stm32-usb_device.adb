...
package body STM32.USB_Device is
   ...
   begin
      ...
      for Ep in EPR_Registers'First + 1 .. EPR_Registers'Last loop
         ...
         begin
            EPRS (Ep) :=
              (Get_EPR_With_Invariant
                 (Cur)with delta STAT_RX => Cur.STAT_RX xor 0,
   ...
   begin
      ...
      return
        (Current with delta CTR_RX => True,
        DTOG_RX => False,
        STAT_RX => 0,
        CTR_TX  => True,
        DTOG_TX => False,
   ...
   begin
      ...
      Tmp :=
        ...
        STAT_TX                   =>
          (if EP.Dir = EP_In then Cur.STAT_TX xor 2 else 0),
        STAT_RX =>
          (if EP.Dir = EP_Out then Cur.STAT_RX xor 2 else 0));
   ...
   begin
      ...
      if Ready then
         ...
         UPR :=
           (Get_EPR_With_Invariant
              (Cur)with delta STAT_RX => Cur.STAT_RX xor 3);
      else
         --  Set to NAK if not ready (why is it needed??)
         UPR :=
           (Get_EPR_With_Invariant
              (Cur)with delta STAT_RX => Cur.STAT_RX xor 2);
      end if;
   ...
   begin
      ...
      case Ep.Dir is
         when USB.EP_In =>
            UPR :=
              (Get_EPR_With_Invariant
                 (Cur)with delta STAT_TX => Cur.STAT_TX xor V);

            if not Set then
               UPR :=
                 (Get_EPR_With_Invariant
                    (Cur)with delta DTOG_TX => False xor Cur.DTOG_TX);
            end if;

         when USB.EP_Out =>
            UPR :=
              (Get_EPR_With_Invariant
                 (Cur)with delta STAT_RX => Cur.STAT_RX xor V);
            if not Set then
               UPR :=
                 (Get_EPR_With_Invariant
                    (Cur)with delta DTOG_RX => False xor Cur.DTOG_RX);
            end if;
      end case;
      --      EndLog ("< EP_Stall");
   end EP_Stall;
